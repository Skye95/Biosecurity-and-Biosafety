...
<body>
                ...
                <?php $i=0; foreach($all_formf as $row): ?>
                    <tr class="searchable">
                        ...
                        <td><?php 
                            ...
                            ?></td>
                        <td><button type="button" name = 'formf_load' value = 'Load' onclick="location.href='<?php echo site_url().'/formf/load_form?id='.$row->account_id;?>'" class="btn btn-primary">Load</button></td>
                        <!--
            ...
            <table class="table table-hover table-bordered">
                ...
                <?php $i=0; foreach($all_formf_SSBC as $row): ?>
                    <tr class="searchable">
                        ...
                        -->
                        <td class="text-center">
                            <i class="btn btn-success fa fa-check" onclick="approve2(<?php echo $row->account_id; ?>)" title="Approve"></i>
                            <hr/>
                            <i class="btn btn-danger fa fa-times" onclick="reject2(<?php echo $row->account_id; ?>)" title="Reject"></i>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
    ...
    <script>
        function approve(i){
            window.location = "<?php echo base_url(); ?>index.php/export_LMO_approval/approve/" + i;
        }
        
        function reject(i){
            var j = prompt("Reason for Rejecting:", "Did not meet requirement");
            if (j != null) {
                window.location = "<?php echo base_url(); ?>index.php/export_LMO_approval/reject/" + i + "/" + btoa(j);
            }
        }
    </script>
    ...
    <script>
        function approve2(i){
            window.location = "<?php echo base_url(); ?>index.php/export_LMO_approval/approve2/" + i;
        }
        
        function reject2(i){
            var j = prompt("Reason for Rejecting:", "Did not meet requirement");
            if (j != null) {
                window.location = "<?php echo base_url(); ?>index.php/export_LMO_approval/reject2/" + i + "/" + btoa(j);
            }
        }
    </script>
</body>
